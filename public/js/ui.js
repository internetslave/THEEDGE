...
function freshnessTone(status) {
  if (status === 'fresh') return '#7ef7d2';
  if (status === 'cached') return '#f6ad55';
  return '#fca5a5';
}

function trustTone(sourceType) {
  if (sourceType === 'verified_data') return { label: 'Verified data', color: '#7ef7d2' };
  if (sourceType === 'projected_data') return { label: 'Projected values', color: '#f6ad55' };
  if (sourceType === 'generated_insight') return { label: 'Generated insight', color: '#a78bfa' };
  return { label: 'Reference data', color: '#9fb0c8' };
}

function buildMarketTrustMeta(meta = {}) {
  if (meta && meta.provider) return meta;
  return {
    provider: 'EdgeIQ market snapshot',
    sourceType: 'verified_data',
    freshnessStatus: oddsFreshnessStatus,
    freshnessLabel: oddsFreshnessText,
    asOf: oddsAsOf || oddsLastUpdated,
    asOfIso: oddsAsOf ? new Date(oddsAsOf).toISOString() : null,
    sourceAttribution: 'Market pricing supplied via connected odds feeds.',
  };
}

function buildAnalysisTrustMeta(meta = {}, fallbackConfidence = 0) {
  if (meta && meta.provider) return meta;
  return {
    provider: 'EdgeIQ AI analysis',
    sourceType: 'generated_insight',
    generatedAt: Date.now(),
    confidenceBand: fallbackConfidence ? `${fallbackConfidence}% confidence signal` : 'Generated insight',
    uncertaintyNote: 'Model output should be reviewed alongside price movement and late news.',
    sourceAttribution: 'Generated by EdgeIQ from the available inputs.',
  };
}

function resultStateLabel(result) {
  if (result === 'PENDING') return 'Pending settlement';
  if (result === 'WIN') return 'Settled win';
  if (result === 'LOSS') return 'Settled loss';
  if (result === 'PLACE') return 'Settled place';
  if (result === 'PUSH') return 'Settled push';
  return 'Tracked result';
}

function renderTrustPattern(meta = {}, { title = '', compact = false, showSourceAttribution = true } = {}) {
  const trust = meta || {};
  const tone = trustTone(trust.sourceType);
  const freshness = trust.freshnessStatus ? `
    <span class="trust-chip" style="--trust-color:${freshnessTone(trust.freshnessStatus)}">
      ${trust.freshnessStatus === 'fresh' ? 'Fresh' : trust.freshnessStatus === 'cached' ? 'Cached' : 'Stale'} · ${trust.freshnessLabel || ''}
    </span>` : '';
  const generatedAt = trust.generatedAt ? `
    <span class="trust-chip" style="--trust-color:#9fb0c8">Generated ${formatRelativeTime(trust.generatedAt)}</span>` : '';
  const asOf = trust.asOf || trust.asOfIso ? `
    <div class="trust-meta-line"><strong>Timestamp</strong><span>${formatAbsoluteTime(trust.asOf || trust.asOfIso)}</span></div>` : '';
  const attribution = showSourceAttribution && trust.sourceAttribution ? `
    <div class="trust-note">${trust.sourceAttribution}</div>` : '';
  const uncertainty = trust.uncertaintyNote ? `<div class="trust-note">${trust.uncertaintyNote}</div>` : '';
  const warning = trust.warning ? `<div class="trust-warning">${trust.warning}</div>` : '';

  return `
    <div class="trust-pattern${compact ? ' compact' : ''}">
      ${title ? `<div class="trust-title">${title}</div>` : ''}
      <div class="trust-chip-row">
        <span class="trust-chip" style="--trust-color:${tone.color}">${tone.label}</span>
        ${trust.provider ? `<span class="trust-chip" style="--trust-color:#60a5fa">${trust.provider}</span>` : ''}
        ${freshness}
        ${generatedAt}
        ${trust.cached ? '<span class="trust-chip" style="--trust-color:#f6ad55">Cached snapshot</span>' : ''}
      </div>
      <div class="trust-meta">
        ${trust.confidenceBand ? `<div class="trust-meta-line"><strong>Confidence framing</strong><span>${trust.confidenceBand}</span></div>` : ''}
        ${asOf}
      </div>
      ${warning}
      ${uncertainty}
...
function snapshotInsight(match) {
  return {
    id: String(match.id),
    sport: match.sport,
    home: match.home,
    ...
    freshnessStatus: match.dataMeta?.freshnessStatus || oddsFreshnessStatus,
    commenceTime: match.commenceTime || null,
    savedAt: new Date().toISOString(),
  };
}

function isInsightSaved(matchId) {
  return savedInsights.some((item) => String(item.id) === String(matchId));
}

function toggleSavedInsight(matchId) {
  const existingIndex = savedInsights.findIndex((item) => String(item.id) === String(matchId));
  if (existingIndex >= 0) {
    const removed = savedInsights[existingIndex];
    savedInsights.splice(existingIndex, 1);
    persistSavedInsights();
    showToast(`Removed ${removed.pick || 'signal'} from saved insights`, false);
  } else {
    const match = UPCOMING.find((item) => String(item.id) === String(matchId));
    if (!match) {
      showToast('Could not save that signal right now', true);
      return;
    }
    savedInsights.unshift(snapshotInsight(match));
    savedInsights = savedInsights.slice(0, 18);
    persistSavedInsights();
    showToast('Saved to your insight stack', false);
  }
...
function openPlanOverview() {
  showPage('analysis');
  setTimeout(() => {
    document.getElementById('analysis-plan-hook')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 120);
}

function setAdvancedSignalFilter(key, value, page = 'tips') {
  advancedSignalFilters[key] = value;
  if (page === 'upcoming') {
    renderUpcoming();
    return;
  }
...
function applyAdvancedSignalFilters(matches) {
  return matches.filter((match) => {
    if (advancedSignalFilters.confidence === 'high' && (match.confidence || 0) < 75) return false;
    if (advancedSignalFilters.confidence === 'plus' && (match.confidence || 0) < 65) return false;
    if (advancedSignalFilters.edge === 'value' && !match.valueBet) return false;
    if (advancedSignalFilters.edge === 'ai' && !match.aiReady) return false;
    if (advancedSignalFilters.freshness === 'fresh' && match.dataMeta?.freshnessStatus !== 'fresh') return false;
    if (advancedSignalFilters.freshness === 'trusted' && match.dataMeta?.freshnessStatus === 'stale') return false;
    return true;
  });
}
  ...
  try {
    ...
    if (data.success && data.analysis) {
      ...
      if (idx !== -1) {
        ...
        UPCOMING[idx].analysisMeta = data.analysis._meta || UPCOMING[idx].analysisMeta || null;
        UPCOMING[idx].disclaimer = data.analysis.disclaimer || UPCOMING[idx].disclaimer || '';
      }
      stopFactCycler();
      ...
      if (el) {
        if (res.status === 401) {
          ...
          el.textContent = data?.error || '🤖 AI analysis is temporarily unavailable — try again later';
        }
      }
    }
  } catch(e) {
    ...
    const el = document.getElementById('md-ai-loading');
    if (el) el.textContent = '🤖 AI analysis is temporarily unavailable — check connection';
  }
}
...
function openQuickBet(matchId) {
  ...
  if (m.isRacing && m.runners && m.runners.length) {
    ...
    document.getElementById('qb-pick-label').textContent = 'MODEL LEAN';
